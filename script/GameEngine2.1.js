...
function NewGame(canvasid, title) {
    ...
    this.startGame = function (canvasid) {
		this.player = new Player(192, 224, this);
 ...
 * *****/
function Player(coordX, coordY, currgame) {
    this.x = coordX;
    this.y = coordY;
	let img = new Image();
	img.src ="http://www.extremetech.com/wp-content/uploads/2011/06/firefox-logo-huge.jpg";
	this.draw = function(){
		var ctx = currgame.context; //game window
        ctx.fillStyle = "#b81eac";
        ctx.beginPath();
        ctx.arc(this.x, this.y, brad, 0, 2*Math.PI);
        ctx.fill();
        ctx.strokeStyle = color;
